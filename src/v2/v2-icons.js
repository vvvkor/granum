...
const icons = [
  ...
  ['world', 24, 'M12 2a10 10 0 1 0 .01 0zm0 2a8 8 0 1 1-.01 0za6 8 0 1 0 .01 0za4.4 8 0 1 1-.01 0za1 8 0 1 0 .01 0zm8 8a8 .4 0 1 0 0 .01z'],
  ...
  ['back', 10, 'M6 1l1 1-3 3 3 3-1 1-4-4z'],
  ['forward', 10, 'M4 1l4 4-4 4-1-1 3-3-3-3z'],
...
const scale = (s, src, trg=16, dec=100) => s.replace(/\.\d+|\d+(\.\d+)?/g, n => (Math.ceil(parseFloat(n) / src * trg * dec) / dec))

console.log(icons
  // scale coords
  .map(n => [n[0], n[1], scale(n[2], n[1])])
  // generate css
  //.map(n => `.icon-${n[0]} {--w:${n[1]};--p:path("${n[2]}");}`)
  .map(n => `.icon-${n[0]} {--p:path("${n[2]}");}`)
  .join('\n'));